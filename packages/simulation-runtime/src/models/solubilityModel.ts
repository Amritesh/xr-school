...
export const SOLUBILITY_SUBSTANCES: Record<
  SubstanceId,
  SubstanceDefinition
> = {
  salt: {
    ...
    color: '#dff5ff',
    densityGPerMl: 2.16,
    particleRadiusMm: 0.35,
    solubilityAt25GPer100GWater: 36,
    solubilityTemperatureSlope: 0.045,
    dissolutionRatePerSecond: 0.16,
    suspensionFractionWhenStirred: 0,
    settlingRatePerSecond: 0,
    turbidityStrength: 0.08,
    explanation:
      'Salt forms a homogeneous solution. Its ions remain present even when no grains are visible.',
  },
  sugar: {
    id: 'sugar',
    label: 'Sugar',
    ...
    color: '#fff1ba',
    densityGPerMl: 1.59,
    particleRadiusMm: 0.65,
    solubilityAt25GPer100GWater: 211,
    solubilityTemperatureSlope: 2.35,
    dissolutionRatePerSecond: 0.075,
    suspensionFractionWhenStirred: 0,
    settlingRatePerSecond: 0,
    turbidityStrength: 0.06,
    explanation:
      'Sugar molecules separate and spread uniformly through water without becoming ions.',
  },
  sand: {
    id: 'sand',
    label: 'Sand',
    formula: 'mostly SiO₂',
    kind: 'insoluble-solid',
    color: '#d3a25e',
    densityGPerMl: 2.65,
    particleRadiusMm: 0.55,
    solubilityAt25GPer100GWater: 0,
    solubilityTemperatureSlope: 0,
    dissolutionRatePerSecond: 0,
    suspensionFractionWhenStirred: 0.58,
    settlingRatePerSecond: 0.13,
    turbidityStrength: 0.52,
    explanation:
      'Sand grains may be carried while water moves, but they settle and remain a separate solid.',
  },
  chalk: {
    id: 'chalk',
    label: 'Chalk powder',
    ...
    color: '#f4f5ed',
    densityGPerMl: 2.7,
    particleRadiusMm: 0.08,
    solubilityAt25GPer100GWater: 0,
    solubilityTemperatureSlope: 0,
    dissolutionRatePerSecond: 0,
    suspensionFractionWhenStirred: 0.92,
    settlingRatePerSecond: 0.035,
    turbidityStrength: 1,
    explanation:
      'Fine chalk makes a cloudy suspension. It settles much more slowly than coarse sand.',
  },
  oil: {
    id: 'oil',
    label: 'Cooking oil',
    formula: 'mixture of lipids',
    kind: 'immiscible-liquid',
    color: '#f2c94c',
    densityGPerMl: 0.91,
    particleRadiusMm: 0.4,
    solubilityAt25GPer100GWater: 0,
    solubilityTemperatureSlope: 0,
    dissolutionRatePerSecond: 0,
    suspensionFractionWhenStirred: 0.82,
    settlingRatePerSecond: 0.18,
    turbidityStrength: 0.62,
    explanation:
      'Oil and water are immiscible. Stirring breaks oil into droplets temporarily; droplets coalesce and rise again.',
  },
  sawdust: {
    id: 'sawdust',
    label: 'Dry sawdust',
    formula: 'cellulose-rich wood particles',
    kind: 'floating-solid',
    color: '#9a6337',
    densityGPerMl: 0.35,
    particleRadiusMm: 1.1,
    solubilityAt25GPer100GWater: 0,
    solubilityTemperatureSlope: 0,
    dissolutionRatePerSecond: 0,
    suspensionFractionWhenStirred: 0.62,
    settlingRatePerSecond: 0.22,
    turbidityStrength: 0.36,
    explanation:
      'Dry sawdust is insoluble. Many particles float because their average density is below water and they may trap air.',
  },
};
...
function approach(
  ...
  return (
    current +
    (target - current) * (1 - Math.exp(-ratePerSecond * deltaSeconds))
  );
}

export function createSolubilityModel(
  config: SolubilityModelConfig = {},
): SolubilityModel {
  const waterMassG = clamp(finite(config.waterMassG ?? 200, 'water mass'), 50, 500);
  let substanceId = config.substanceId ?? 'salt';
  let temperatureC = clamp(
    finite(config.temperatureC ?? 25, 'temperature'),
    5,
    70,
  );
  let stirring = false;
  ...
  function definition() {
    return SOLUBILITY_SUBSTANCES[substanceId];
  }

  function capacity() {
    const item = definition();
    if (item.kind !== 'soluble-solid') return 0;
    const per100G = Math.max(
      0,
      item.solubilityAt25GPer100GWater +
  ...
  function phaseState(): MixtureClass {
    ...
    if (item.kind === 'immiscible-liquid') {
      return suspendedMassG > addedMassG * 0.12
        ? 'emulsion'
        : 'separated-layer';
    }
    if (item.kind === 'floating-solid') {
      return floatingMassG > addedMassG * 0.5
        ? 'floating-solid'
        : 'suspension';
    }
  ...
  function snapshot(): MixtureSnapshot {
    ...
    const saturationPercent =
      applicable && cap > 0
        ? clamp((dissolvedMassG / cap) * 100, 0, 100)
        : 0;
    const saturationState: SaturationState = !applicable
      ? 'not-applicable'
      : saturationPercent >= 99.5
        ? 'saturated'
    ...
    return {
      ...
      saturationPercent,
      saturationState,
      turbidityPercent: clamp(
        turbidityBase * definition().turbidityStrength * 125,
        0,
        100,
      ),
  ...
  return {
    addSolute(massG) {
      ...
      addedMassG += massG;
      if (definition().kind === 'immiscible-liquid') {
        separatedMassG += massG;
      } else if (definition().kind === 'floating-solid') {
        floatingMassG += massG;
      } else {
        settledMassG += massG;
      }
    ...
    step(deltaSeconds) {
      ...
      if (deltaSeconds < 0 || deltaSeconds > 0.25) {
        throw new Error('delta seconds must be between 0 and 0.25');
      }
      ...
      if (item.kind === 'soluble-solid') {
        ...
        const temperatureFactor = clamp(
          1 + (temperatureC - 25) * 0.018,
          0.55,
          1.9,
        );
        const agitationFactor = stirring ? 4.2 : 1;
        const rate =
          item.dissolutionRatePerSecond * temperatureFactor * agitationFactor;
        const transfer =
          transferable * (1 - Math.exp(-rate * deltaSeconds));
        ...
        const targetSuspended = stirring
          ? addedMassG * item.suspensionFractionWhenStirred
          : 0;
        const rate = stirring ? 0.9 : item.settlingRatePerSecond;
        suspendedMassG = approach(
          suspendedMassG,
          targetSuspended,
          rate,
          deltaSeconds,
      ...
      const massError =
        addedMassG - pools.reduce((sum, value) => sum + value, 0);
...
> = (state, action) => {
  if (action.actionId === 'solubility.predict') {
    const { id, encodedValue } = actionParts(action);
    const value =
      typeof action.value === 'string' ? action.value : encodedValue;
    if (value !== 'soluble' && value !== 'insoluble') {
      throw new Error(`Prediction for ${id} must be soluble or insoluble`);
    }
    return {
      state: {
        ...state,
        predictions: { ...state.predictions, [id]: value },
        lastSubstanceId: id,
      },
  ...
  if (action.actionId === 'solubility.run-fair-trial') {
    const { id } = actionParts(action);
    if (!state.predictions[id]) {
      throw new Error(`Record a prediction for ${id} before running its fair trial`);
    }
